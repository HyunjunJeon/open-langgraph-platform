...
# Internal context variable: stores current authentication context (None if not set)
# Use asyncio-friendly contextvars instead of thread-locals
...
def get_auth_ctx() -> BaseAuthContext | None:
    ...
    Returns:
        Auth.types.BaseAuthContext | None: The authentication context if set, otherwise None.

    Usage Example:
        # Check user in a graph node
...
async def with_auth_ctx(
    ...
    """Temporarily set the authentication context for the duration of an async block.

    This context manager injects authentication information during LangGraph graph execution.
    It automatically restores the previous context upon exiting the block.

    Parameters
    ----------
    user : BaseUser | None
        The authenticated user (None for anonymous access).
    permissions : list[str] | AuthCredentials | None
        A Starlette AuthCredentials instance or a list of permission strings.
        None means no permissions.

    Usage Example:
        async with with_auth_ctx(user, ["read", "write"]):
            # Inside this block, user information can be accessed with get_auth_ctx()
